...
uint8_t ArgoUlisseClimate::calc_checksum_wrem2_(const ArgoProtocolWREM2 *data,
                                                size_t length) {
  ESP_LOGV(TAG, "Calculating WREM-2 checksum..");
  if (length < 2)
    return 0;
  uint8_t checksum = ARGO_WREM2_POST;
...
void ArgoUlisseClimate::transmit_state() {
  if (this->protocol_version_ == ARGO_PROTOCOL_WREM2) {
    this->transmit_state_wrem2();
  } else {
    this->transmit_state_wrem3();
  }
}

void ArgoUlisseClimate::transmit_state_wrem3() {
  ESP_LOGD(TAG, "Transmitting WREM-3 state..");
  ...
  ac_packet.IrChannel = this->channel_;
  ...
  ac_packet.Night = this->preset.value_or(climate::CLIMATE_PRESET_NONE) ==
                    climate::CLIMATE_PRESET_SLEEP;
  ac_packet.Eco = this->preset.value_or(climate::CLIMATE_PRESET_NONE) ==
                  climate::CLIMATE_PRESET_ECO;
  ac_packet.Boost = this->preset.value_or(climate::CLIMATE_PRESET_NONE) ==
                    climate::CLIMATE_PRESET_BOOST;
  ac_packet.Filter = this->filter_;
  ac_packet.Light = this->light_ &&
  ...
  ac_packet.Sum =
      this->calc_checksum_(&ac_packet, ARGO_IR_MESSAGE_LENGTH_AC_CONTROL);

  // Transmit the IR signal
  auto transmit = this->transmitter_->transmit();
  auto *data = transmit.get_data();
  data->set_carrier_frequency(
      ARGO_IR_FREQUENCY); // Adjust if the frequency is different
...
void ArgoUlisseClimate::transmit_state_wrem2() {
  ESP_LOGD(TAG, "Transmitting WREM-2 state..");
  this->last_transmit_time_ = millis();
  ArgoProtocolWREM2 ac_packet;
  memset(&ac_packet, 0, sizeof(ac_packet));

  ac_packet.Pre1 = ARGO_WREM2_PREAMBLE1;
  ac_packet.Pre2 = ARGO_WREM2_PREAMBLE2;

  ac_packet.Mode = this->operation_mode_();
  ac_packet.RoomTemp = this->sensor_temperature_();
  ac_packet.Temp = this->temperature_();
  ac_packet.Fan = this->fan_speed_();
  ac_packet.Flap = this->swing_mode_();
  ...
  ac_packet.Night = this->preset.value_or(climate::CLIMATE_PRESET_NONE) ==
                    climate::CLIMATE_PRESET_SLEEP;
  ac_packet.Max = this->preset.value_or(climate::CLIMATE_PRESET_NONE) ==
                  climate::CLIMATE_PRESET_BOOST;

  ac_packet.Post = ARGO_WREM2_POST;
  ac_packet.Sum =
      this->calc_checksum_wrem2_(&ac_packet, ARGO_WREM2_STATE_LENGTH);

  auto transmit = this->transmitter_->transmit();
  auto *data = transmit.get_data();
  data->set_carrier_frequency(ARGO_IR_FREQUENCY);

  data->mark(ARGO_HEADER_MARK);
  data->space(ARGO_HEADER_SPACE);

  for (uint8_t i = 0; i < ARGO_WREM2_STATE_LENGTH; i++) {
    for (uint8_t mask = 1; mask > 0; mask <<= 1) {
      data->mark(ARGO_BIT_MARK);
      bool bit = ac_packet.raw[i] & mask;
      data->space(bit ? ARGO_ONE_SPACE : ARGO_ZERO_SPACE);
...
void ArgoUlisseClimate::transmit_ifeel_wrem3() {
  ...
  ESP_LOGD(TAG, "Transmitting WREM-3 iFeel report..");
  this->last_transmit_time_ = millis();
  ...
  ifeel_packet.IrChannel = this->channel_;
  ...
  ifeel_packet.ifeelreport.CheckHi = this->calc_checksum_(
      &ifeel_packet, ARGO_IR_MESSAGE_LENGTH_IFEEL_TEMP_REPORT);
...
void ArgoUlisseClimate::transmit_ifeel_wrem2() {
  ESP_LOGD(TAG, "Transmitting WREM-2 iFeel report..");
  this->last_transmit_time_ = millis();
  ...
  ifeel_packet.ifeelreport.SensorT = temp;
  ...
  for (uint8_t i = 0; i < ARGO_WREM2_IFEEL_REPORT_LENGTH; i++) {
    for (uint8_t mask = 1; mask > 0; mask <<= 1) {
      data->mark(ARGO_BIT_MARK);
      bool bit = ifeel_packet.raw[i] & mask;
      data->space(bit ? ARGO_ONE_SPACE : ARGO_ZERO_SPACE);
...
climate::ClimateTraits ArgoUlisseClimate::traits() {
  climate::ClimateTraits
      traits; // Do not inherit from ClimateIR to drop the default modes
...
void ArgoUlisseClimate::control(const climate::ClimateCall &call) {
