...
// WREM-2 Constants
const uint8_t ARGO_WREM2_PREAMBLE1 = 0b10101100; // 0xAC
const uint8_t ARGO_WREM2_PREAMBLE2 = 0b11110101; // 0xF5
const uint8_t ARGO_WREM2_POST = 0b00000010; // 0x02
const uint8_t ARGO_WREM2_SENSOR_FIXED = 0b011; // 3
const uint8_t ARGO_WREM2_SENSOR_CHECK = 52;
const size_t ARGO_WREM2_STATE_LENGTH = 12;
const size_t ARGO_WREM2_IFEEL_REPORT_LENGTH = 4;

#pragma pack(push, 1)  // Add a packing directive to ensure the structure is tightly packed:
union ArgoProtocolWREM2 {
  uint8_t raw[ARGO_WREM2_STATE_LENGTH];  ///< The state in native IR code form
  struct {
    // Byte 0
    uint64_t Pre1     :8;
    // Byte 1
...
class ArgoUlisseClimate : public climate_ir::ClimateIR {
 public:
  ...
  ArgoProtocolVersion get_protocol_version() { return this->protocol_version_; }
 ...
 protected:
  ...
  void transmit_state_wrem2();
  void transmit_state_wrem3();
  void transmit_ifeel_();
  void transmit_ifeel_wrem2();
  void transmit_ifeel_wrem3();
  uint8_t calc_checksum_(const ArgoProtocolWREM3 *data, size_t length);
  uint8_t calc_checksum_wrem2_(const ArgoProtocolWREM2 *data, size_t length);
